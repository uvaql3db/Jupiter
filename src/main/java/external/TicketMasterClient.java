package external;
//api key: iDUe6uGTHpVDhTWD5xAJmiqyqZZ5HDoc
...
public class TicketMasterClient {
	...
	private static final String API_KEY = "iDUe6uGTHpVDhTWD5xAJmiqyqZZ5HDoc";
	...
	public List<Item> search(Double lat, double lon, String keyword) {
		...
		try {
			...
			//object refered to by the URL. The HttpUrlConnection class allows us to perform
			//basic http requests without the use of additional libraryes
			HttpURLConnection connection = (HttpURLConnection) new URL(url).openConnection();
			connection.setRequestMethod("GET");
			connection.connect();
			
			//get the status code from an http response, can use getResponseCode(), connect()
			//getInputStream(), getOutputStream methods
			int responseCode = connection.getResponseCode();
			System.out.println("Sending fra to url:" + url);
	...
	private List<Item> getItemList(JSONArray events) throws JSONException {
		...
		for (int i = 0; i < events.length(); i++) {
			...
			ItemBuilder builder = new ItemBuilder();//内部类是public的话也能直接initiate
			if (!event.isNull("id")) {
	...
	//helper methods: fetch data fields included deeply in response body
	//address
